...
function parseIssueArguments(argumentsText: string): Record<string, unknown> {
  let parsed: unknown
  try {
  ...
  if (Object.keys(params).some((key) => !allowed.has(key))) {
    throw new Error('GitHub issue arguments contain an unsupported field.')
  }
  if (
    typeof params.repo !== 'string' ||
    !/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(params.repo)
  ) {
    throw new Error('The repository must use owner/name form.')
  }

  const state = params.state ?? 'open'
  if (state !== 'open' && state !== 'closed' && state !== 'all') {
    throw new Error('The issue state is not supported.')
  }
  const sort = params.sort ?? 'updated'
  if (sort !== 'updated' && sort !== 'created') {
    throw new Error('The issue sort is not supported.')
  }
  ...
  if (
    ...
    throw new Error('The issue limit must be an integer between 1 and 50.')
  }
...
export const localAgentToolProvider: LocalToolProvider = {
  ...
  invoke: async (call, signal) => {
    if (call.function.name !== ISSUES_TOOL.function.name) {
    ...
    const params = parseIssueArguments(call.function.arguments)
    ...
    if (result.status !== 'succeeded' || typeof result.stdout !== 'string') {
      ...
      throw new Error(detail || 'GitHub CLI could not read the repository.')
    }
    ...
    return JSON.stringify({
      source: 'local gh cli',
      operation: result.operation ?? 'github.issues.list',
