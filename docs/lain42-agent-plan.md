...
- 回归测试覆盖 JSON 参数键顺序不同的重复查询、8 轮后的工具禁用总结，以及模型仍请求工具时的有界结果回退。完整前端测试 75 个文件/463 项通过，类型检查、变更文件 lint 与格式检查通过；没有在本机执行生产构建。
...
- 正式站用同一搜索请求复测后 15.82 秒返回答案，没有出现“未批准”或步数上限错误；但返回内容引用了与 Rust 官方博客问题无关的 C++ 论文。因此此提交修复的是工具循环可恢复性，不代表搜索召回准确或覆盖通用网页；下一步需要改进来源选择/相关性，并明确告知模型当前搜索索引边界。
