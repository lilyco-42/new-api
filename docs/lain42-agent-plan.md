...
日期：2026-09-21。基线：`d74fd44`，分支 `agent-ui`。状态：预研完成、待实现；本文不代表功能已经上线。
...
| `tauri/TOOL_PROTOCOL.md` | v1 边界文档 | MCP 会话尚未实现；对输出上限与隔离的描述需要在实现时修正 |
...
| P0-D | rmcp stdio/HTTPS 适配、用户连接设置、写操作批准与执行去重 | P0-A 契约 | CLI/MCP 共用聊天循环；批准/拒绝均有可见结果 |
...
| A5 | 写操作批准 | 在一次性测试仓库验证：未批准/拒绝零写入；批准一次只执行一次；篡改参数/重放失败 |
| A6 | MCP 两种传输 | stdio 与 Streamable HTTP 各完成 list/call/断线/取消；错误与 schema 不兼容可解释 |
...
下一步唯一实施起点：P0-A，提取受限操作执行核心，并用“长输出、挂起取消、两档案 token 冲突”三个回归场景验证，再开始模型续轮。
