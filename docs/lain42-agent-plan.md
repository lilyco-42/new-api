...
- `local-tool-loop` 现按规范化参数识别重复的 `web.search`，同一查询只运行一次，然后禁用工具请求模型总结。达到总工具/轮次上限时也执行一次无工具总结；若模型仍返回工具调用或总结失败，则返回有界的已收集原始结果，不再把已有搜索结果丢成通用步数错误。
- 回归测试覆盖 JSON 参数键顺序不同的重复查询、8 轮后的工具禁用总结，以及模型仍请求工具时的有界结果回退。完整前端测试 75 个文件/467 项通过，类型检查通过；此前线上复测只验证了循环恢复，没有证明检索结果相关。
- `b98c451` 的 GitHub Actions `35924562609` 全绿：前端生产构建、前端测试、Agent API 测试、Linux amd64 构建和产物上传通过。部署使用的二进制 SHA-256 为 `5bb338152efe465f91ffe6be858e0bfdccb82ca856922b66d24806e111010c4f`，与 CI 日志和上传到服务器的文件一致；线上状态为 `agent-b98c451`、容器 healthy，`/agent` 返回 HTTP 200，PostgreSQL/Redis 持续运行。生产二进制只由 Actions 构建，本机未构建。
- 正式站用同一搜索请求复测后 15.82 秒返回答案，没有出现“未批准”或步数上限错误；但返回内容引用了与 Rust 官方博客问题无关的 C++ 论文。本地改进搜索来源选择/相关性，不代表通用互联网搜索已实现。
- 搜索来源按意图路由：普通技术发现查询 GitHub/Hugging Face，只有明确论文或学术研究查询才查询 OpenAlex；OpenAlex 结果需要和查询关键词达到最小重合，否则过滤并说明。直接搜索 RustCC、CodeReset、GHFind、博客或社区页面时，工具返回“暂无对应索引适配器”，不会拿无关论文或仓库冒充命中；用户仍可显式选择单一索引或全部已支持索引。WASM 页面读取依旧受目标站 CORS 约束。
- 全部变更文件的 Oxlint、Oxfmt 检查通过，全部 locale JSON 可解析。仓库全量 `format:check` 仍因 41 个既有且与本次无关的文件失败；没有在本机执行生产构建。
- `ee94635` 已推送到 `agent-ui`；GitHub Actions `35928287302` 的前端构建、467 项前端测试、Agent API 测试及 Linux amd64 构建均通过。CI 二进制 SHA-256 为 `48ea5bf3d45a2bd5d7299f6e0a3e151465e0eabac8f57092b867e27a5679cc66`。
- `agent-ee94635` 已上线；从生产容器可写层校验到的文件大小与 CI 校验和完全一致，容器 healthy，`/api/status` 版本正确、`/agent` 返回 HTTP 200，PostgreSQL/Redis 保持运行。服务器根盘目前只剩约 30 MB，后续再次发布前仍需先处理容量。
...
## 0.8 工作区文件上限与 OAuth 重复授权竞态（2026-09-24）

- `ad3b999` 为工作区文件增加明确边界：最多 10 个、单个最多 8 MiB，文本预览通过 `Blob.slice` 只读取前 120 KB；普通聊天附件沿用同一个单文件上限。新增边界测试覆盖超大文件、数量上限、文本截断和二进制文件不转文本。
...
## 0.5 WebFetch 与用户固定知识库（2026-09-23）

- 本地代码已增加 `web.fetch`：读取用户指定的公开网页；搜索仍走 `web.search`。后端限制协议、目标端口、DNS 公网地址、跳转、耗时、响应体和文本类型，并将页面内容视作不可信输入。发布前仍需部署并跑线上回归。
- 桌面 MCP 面板代码增加公开 DeepWiki 一键连接，可查询公开 GitHub 仓库的结构、页面内容和问答；部署后网页/手机可直接使用通用 `web.fetch`，不依赖桌面或 Radxa 在线。公共 DeepWiki 不用于私有仓库。
- 后续知识库按“每用户一个固定空间”设计：普通账号 5 GB，会员 20 GB（十进制字节，用户权益由服务端判定）；用户显式固定网页或公开仓库后，才将当前来源快照写入该用户私有 OSS 前缀。按需 WebFetch 仍不自动入库。
- 入库时 SQL 保存来源 URL、仓库/ref、抓取时间、ETag/内容哈希、对象 key 和当前逻辑字节数。服务端原子预留配额；相同内容去重，更新替换当前对象，写入失败释放预留；超额拒绝并提供已用/上限信息。历史版本和搜索索引不得绕过配额与清理策略。
- 当前仓库的 S3/OSS artifact store 仍是禁用占位配置，因此知识库持久化、配额和定时同步都尚未上线。首阶段先完成通用 WebFetch；下一阶段才实现按用户固定来源的 OSS 快照及可关闭的定时刷新，不抓取/镜像整个 DeepWiki。
- 修复了只读 `web.search` / GitHub 工具经聚合器调用时被默认判为“未批准”的错误；只读工具不再因缺少审批回调而失败，MCP 仍执行逐次审批。模型生成越界 `limit` 时服务端请求参数按安全上下限截断，格式错误时退回默认值；短数字/问候语不应触发工具调用。
- 具体选项与验证门槛见 `.agents/results/architecture/architecture-recommendation-deepwiki-knowledge.md`。
...
- 模型返回 429 时，聊天错误卡片提供直达模型选择器的操作。不会静默切到可能收费或能力不同的模型。
- 本轮前端 TypeScript 检查和 Rsbuild 生产构建通过；没有完成真实设备配对、模型上游请求或手机浏览器 E2E，所以这些变更不能代替 A1–A11 验收。
- 邮件、QQ、微信和 Telegram 的官方 OAuth/Bot/Webhook 连接器仍未实现；UI 文案或来源链接不等于账号已连接。上线这些连接器前还需要各平台应用配置、最小权限审查和真实回调验证。
...
仍未宣称完成的部分：模型回答级的跨页面断线恢复（当前只恢复工具调用状态，不恢复模型上下文）、MCP 的真实 stdio/HTTPS 服务互操作回归、写操作的持久审批/防重放与全链路 A1–A11 验收。当前 Tauri 配对凭证已按桌面 profile 保存在应用数据目录，MCP 连接仍只在本次桌面进程内存中有效；在迁移到系统密钥库并完成跨平台回归之前，不把它称为企业级持久设备登录。
