...
- 网页搜索现在返回普通网页标题、摘要和链接，默认从 Bing RSS 获取；没有把查询词写入应用日志。可用 `AGENT_WEB_SEARCH_URL` 指向管理员自己的 SearXNG `/search` JSON 接口，避免依赖固定搜索源。线上服务器对 Bing RSS 的连通性探测返回 HTTP 200；该探测不等于部署后的端到端搜索验收。
- 工作区文件现在能附加到下一条消息；图片会作为图片内容传递，文本/代码文件会作为文本内容传递。文件留在浏览器中，不会先上传到 Lain42 文件存储；单文件上限 8 MiB，单条消息最多 5 个附件。移动端现可切换 Tools、Files 和 Preview。
- GitHub 网页 OAuth 的服务器状态显示已启用且 Client ID 已配置；网站前端回调是 `/oauth/github`。尚未替用户完成 GitHub 的最终授权确认，因此不能声称该账号已连接。当前请求的 `repo` scope 可访问私有仓库且权限也包含写操作；Agent 目前只实现读取 API，后续应迁移到权限更细的 GitHub App。
- 邮箱、QQ、微信和 Telegram 的自动回复连接器仍未实现。它们还需要各平台官方应用/机器人配置、回调地址和最小权限方案；自动回复也必须保持逐条审核后发送，不能用页面占位文案代替真实连接。
- 本轮已通过前端 TypeScript 检查、Rsbuild 生产构建和搜索 RSS/JSON 解析的定向 Go 测试；仍未完成账号授权、真实浏览器附件发送、手机 E2E、Radxa 实机配对以及 A1–A11 的全链路验收。
